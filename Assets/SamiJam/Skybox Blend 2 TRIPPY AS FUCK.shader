...
Shader "Skybox Blend 2 TRIPPY AS FUCK" {
Properties {
	...
	Pass {
		...
		half4 frag (v2f IN) : SV_Target
		{
			...
			float3 randomizedColor = float3(
				0.5 + 0.5 * sin((viewDirection.x + _Time.x) *_RandomizeColor.w) * _RandomizeColor.x,
			...
			upDownLerp = pow(upDownLerp + randomizedColor, _Gamma);
			...
			float3 finalColor = lerp(_SkyColorTop, _SkyColorBottom, upDownLerp);
