...
{
    private const string _btnTitle = "<size=14><color=ffffff>Debug Buttons</color></size>";
    ...
    public override void OnInspectorGUI()
    {
        ...
        foreach (var info in _methodCallInfo)
        {
            for (int i = 0; i < info.parameters.Count; i++)
            {
                ...
                // Specific types
                //CheckNull<VRPrefabType>(ref p, f => (VRPrefabType)EditorGUILayout.EnumPopup(fieldName, f));
            }
